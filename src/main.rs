...
mod data {
    const GRID_SIZE: (i16, i16) = (200, 110);
    const GRID_CELL_SIZE: (i16, i16) = (16, 16);
    ...
    pub(crate) const DESIRED_FPS: u32 = 30;
...
impl Fourier_Serie {
    fn init_vectors(constants: Vec<Vec<f32>>, n: i64) -> Vec<Vec<f32>> {
        ...
        if n != 0 {
            for x in 0..n * 2 {
                vectors.push(vec![
                    constants[(x + 1) as usize][0],
                    constants[(x + 1) as usize][1],
                    vectors[x as usize][0],
                    vectors[x as usize][1],
                ])
            }
        }
    ...
    fn calculate_constant(n: i64) -> Vec<f32> {
        ...
        return vec![
            integral.re * data::SCALE_UP_CONST,
            integral.im * data::SCALE_UP_CONST,
        ];
    }
    ...
    fn coordinates_to_screen_coordinates(&mut self, n: i32) -> Vec<f32> {
        ...
        new_vector.push((self.update_pivots(n)[0] * 88.0 * 4.0) + 880.0);

        // y coordinate

        new_vector.push((self.update_pivots(n)[1] * -88.0 * 4.0) + 880.0);
    ...
    fn draw_axis(&mut self, context: &mut Context, canvas: &mut graphics::Canvas) -> Option<i32> {
        ...
        let axis =
            graphics::Mesh::new_line(context, &points_of_real_axis, 10.0, Color::BLUE).ok()?;

        canvas.draw(&axis, Vec2::new(0.0, 880.0));

        let points_of_img_axis = vec![vec2(0.0, 0.0), vec2(0.0, 1760.0)]; // Line from (10, 50) to (100, 50)
        let axis =
            graphics::Mesh::new_line(context, &points_of_img_axis, 10.0, Color::BLUE).ok()?;
    ...
    fn draw_point(&mut self, context: &mut Context, canvas: &mut graphics::Canvas) -> Option<i32> {
        for x in 0..self.constants.len() {
            ...
            canvas.draw(
                &circle,
                Vec2::new(
                    self.coordinates_to_screen_coordinates(x as i32)[0],
                    self.coordinates_to_screen_coordinates(x as i32)[1],
                ),
            );
        }
    ...
    fn draw_function(
        ...
        let xo = graphics::Mesh::new_circle(
            context,
            graphics::DrawMode::fill(),
            vec2(0., 0.),
            10.0,
        ...
        let last_co = self.constants.len() - 1;

        let mutated_coords = self.coordinates_to_screen_coordinates(last_co as i32);

        let clean_coords = vec![
            (mutated_coords[0] - 880.0) / (88.0 * 4.0),
            (mutated_coords[1] - 880.0) / (88.0 * 4.0),
        ];

        println!("{:?}", clean_coords);

        canvas.draw(
            &xo,
            Vec2::new(
                1760.0 + self.t * 200.0,
                self.coords_to_function_coords(clean_coords[0]),
            ),
        );
    ...
    fn coords_to_function_coords(&mut self, i: f32) -> f32 {
        let mut new_coords = i * 200.0 + 880.0;
...
impl event::EventHandler<ggez::GameError> for Fourier_Serie {
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        self.draw_function(ctx, &mut canvas);
...
fn main() -> GameResult {
    ...
    let (ctx, events_loop) = ggez::ContextBuilder::new("Fourier Series", "ObvMamut")
        ...
        .window_mode(
            ggez::conf::WindowMode::default().dimensions(data::SCREEN_SIZE.0, data::SCREEN_SIZE.1),
        )
        .build()?;
